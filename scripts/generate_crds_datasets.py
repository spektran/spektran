#!/usr/bin/env python
"""Generate all CRDS benchmark datasets (train/val/test/heldout)."""
...
def generate_from_config(config_path: Path) -> None:
    ...
    spec = CRDSGenerationSpec(
        ...
        concentration_ppm_low=gas.get("concentration_ppm", {}).get("low", 0.1),
        ...
        path_length_m=gas.get("path_length_m", 0.50),
        matrix_gas=gas.get("matrix_gas", "N2"),
        wavenumber_start_cm1=gas.get("wavenumber_start_cm1", 6046.0),
        wavenumber_end_cm1=gas.get("wavenumber_end_cm1", 6048.0),
        n_spectral_points=gas.get("n_spectral_points", 200),
    )
...
def main():
    configs = [
        "ch4-crds-t1-train-v0",
        "ch4-crds-t1-val-v0",
        "ch4-crds-t1-test-v0",
        "ch4-crds-t3-heldout-v0",
    ]
    for name in configs:
