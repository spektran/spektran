"""Record/config validation against the SPEKTRAN JSON Schemas.

Used by the CLI (``spektran validate``), by CI, and by external data
...
# Repo layout: <root>/src/spektran/validate.py -> <root>/schema/
...
def _load_schema(name: str) -> dict:
    ...
    try:
        ref = resources.files("spektran").joinpath(f"schema/{name}")
        if ref.is_file():
...
def main(argv: list[str] | None = None) -> int:
    ...
    if not args:
        print("usage: python -m spektran.validate RECORD.json [...]", file=sys.stderr)
        return 2
    n_bad = 0
    for arg in args:
        p = Path(arg)
